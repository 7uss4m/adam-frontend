...
export function DataTable<TData, TValue>({
  ...
  return (
    <div>
      <div dir={i18n.language == "en" ? "ltr" : "rtl"} className="flex flex-col py-4 gap-5">
        <DebouncedInput
          value={globalFilter ?? ""}
        ...
        <DropdownMenu dir={i18n.language == "en" ? "ltr" : "rtl"}>
          <DropdownMenuTrigger asChild>
            <Button className="w-fit" variant="outline">{t("status_filter")}</Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-56">
            <DropdownMenuSeparator />
            <DropdownMenuRadioGroup value={filter} onValueChange={setFilter}>
              <DropdownMenuRadioItem value="all">{t("all")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="pinding">{t("pending")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="reject">{t("rejected")}</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="success">{t("succeed")}</DropdownMenuRadioItem>
            </DropdownMenuRadioGroup>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
                    ...
                    <TableCell key={cell.id}>
                      {cell.column.id == "active" ? null : cell.column.id ==
                        ...
                        (row.original as Order).status == "pending" ||
                          ...
                          <Badge className="bg-yellow-500 hover:bg-yellow-500 text-white">{t("pending")}</Badge>
                          : (row.original as Order).status == "reject" ? (
                            <Badge className="hover:pointer-events-none" variant={"destructive"}>{t("rejected")}</Badge>
                          ) : (
                            <Badge className="bg-green-500 text-white hover:bg-green-500 ">{t("succeed")}</Badge>
                          )
                        : cell.column.id == "email" ? (
                          ...
                          `${(row.original as Order)?.total} USD`
                        ) : // <div>
                          ...
                          ) : cell.column.id == "pay_done" ? (row.original as Order).pay_done == false ? t("not_payed") : t("payed") : (
                            flexRender(
