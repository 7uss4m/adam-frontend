...
import getCategoryStats from "../../api/getCategoryStats";
...
import type { CategoryFilterKey, DashboardCategory } from "./category-utils";
...
export default function DashboardCategories() {
  ...
  const filter = (params.get("filter") as CategoryFilterKey) || "all";
  ...
  const categoriesQuery = useQuery({
    queryKey: ["categories", "dashboard", searchQuery, filter],
    queryFn: async () => {
      ...
      return (response.data.result as Category[]) as DashboardCategory[];
    },
    refetchOnWindowFocus: false,
  });
  ...
  const columns = useMemo(
    () => createColumns(t, categoriesQuery),
    [t, categoriesQuery]
  );
  ...
  return (
    <div dir={isRtl ? "rtl" : "ltr"} className="mx-auto max-w-[1600px] space-y-6 px-4 py-6 md:px-6 md:py-8">
      ...
      <motion.section
        ...
        animate={{ opacity: 1, y: 0 }}
        className="relative overflow-hidden rounded-3xl border border-border/40 bg-gradient-to-br from-cyan-600/15 via-card to-violet-600/10 p-6 sm:p-8"
      >
        <div className="pointer-events-none absolute -end-16 -top-16 h-48 w-48 rounded-full bg-cyan-500/20 blur-3xl" />
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <RefreshCw
                className={cn("h-4 w-4", categoriesQuery.isFetching && "animate-spin")}
              />
              {t("refresh") || "تحديث"}
            </Button>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label={t("cat_total")}
          value={stats?.total ?? categories.length}
          sub={t("cat_total_sub")}
      ...
      <section className="space-y-4 rounded-2xl border border-border/50 bg-card/60 p-4 backdrop-blur-sm sm:p-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="ms-auto text-xs text-muted-foreground">
            {categories.length} {t("results") || "نتيجة"}
          </span>
        </div>
      </section>
        ...
        categories.length ? (
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {categories.map((category, i) => (
              ...
              >
                <CategoryCard category={category} query={categoriesQuery} />
              </motion.div>
            ))}
          </div>
        ) : (
          <div className="flex min-h-[240px] flex-col items-center justify-center rounded-2xl border border-dashed border-border/60 bg-card/40 p-8 text-center">
            <Tags className="mb-3 h-10 w-10 text-muted-foreground/50" />
            <p className="font-bold text-foreground">{t("no_results")}</p>
            <p className="mt-1 text-sm text-muted-foreground">{t("cat_empty_hint")}</p>
          </div>
        )
      ) : (
