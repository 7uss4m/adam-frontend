  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent dir={i18n.language == "en" ? "ltr" : "rtl"} className="sm:max-w-[425px] max-h-[100%] overflow-y-auto">
        ...
        <DialogFooter>
          <Button type="submit" disabled={editInfoMutation.isPending} variant={editInfoMutation.isPending ? "ghost" : "default"} onClick={(e) => {
            e.preventDefault()
            editInfoMutation.mutate()
          }}>{editInfoMutation.isPending ? t("saving") : t("save")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
